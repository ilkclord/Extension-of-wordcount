 ...
 * =====================================================================================
 *
 *       Filename:  word-count_tar.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  廿廿一年六月廿六日 十時41分47秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
#define list_for_each_forward(root_ptr, iter)                                 \
    ...
    for (iter = (root_ptr)->next, __ptr = (struct list_entry *) (iter)->next; \
         iter != (root_ptr) && iter != NULL; iter = (typeof((iter))) __ptr,                   \
...
static struct wc_cache  *thread_caches;
static struct wc_cache main_cache ;
...
int wc_init(uint32_t n_threads, uint32_t n_words , int first)
{
...
static int __merge_results(uint32_t tid, uint32_t j, struct wc_cache *wcc)
{
    ...
    for (; iter; iter = ht_get_next(&wcc->htable, j, iter)) {
        ...
        char *wd = GET_WORD(iw);
        struct ht_node *n = ht_find(&cache->htable, wd) ;
        if (n && n != &iw->node_main) { /* block the same node for adding  */
            ...
            iw->counter = 0 ; /*  reinitialize for next time */
        } else if(!n ) /* if word does not exist, then insert the new word */
...
{   
    ...
    printf("fa done\n") ;
    if (wc_init(n_threads, file_size / MAX_WORD_SIZE , first)) return -1;
    printf("wc done\n") ;
    return 0;
}
...
int mr_destroy(void)
{
    ...
    //aka reinitialize the file contnent
...
int queue_list(int max_chunk , queue *q){
    int init = 0 ;
    int jump  = 1 ;
    for(; jump ;init = init + jump ){
        job * j  ;
        info * i ;
        int size ;
        jump =  next_header(&t[init]  , &size)  ;
        if(jump){
           info_init(&i , size , (t[init].mem) , 100) ;
           job_init(&j , i  , NULL) ;
           queue_w(q , j) ;
        }
        else 
...
int main(int argc , char ** argv){
    ...
    t = mmap(NULL , fs , PROT_READ , MAP_PRIVATE , fd , 0) ;
    if(t == MAP_FAILED){
        perror("mmap ERROR") ;
        exit(EXIT_FAILURE) ;
    }
    // create the directory and untar
    if(!create_src(argv[1])){
        perror("src/ file creation deny") ;
        exit(EXIT_FAILURE) ;
    }
    // get the list of the file
    if(!queue_list(fs / 512 , q)){
      perror("queue error") ;
      exit(EXIT_FAILURE) ;
    }
    queue_info(q) ;
    printf("start wordcount \n") ;
    count_start(q) ;
    double end = now() ;
    if(mr_print()){
      perror("printf error\n") ;
    ...
    printf("\nOperating %s with %d files inside \n" ,tar_name , file_n) ;
